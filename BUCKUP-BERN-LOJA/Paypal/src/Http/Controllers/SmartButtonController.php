...
class SmartButtonController extends Controller
{
    ...
    protected function buildRequestBody()
    {
        ...
        $paypalCurrencyCode = $this->smartButton->getPaypalCurrencyCode($cart->cart_currency_code);
        $subTotal = $this->smartButton->convertAmountToPayPalCurrency((float) $cart->sub_total, $cart->cart_currency_code);
        $taxTotal = $this->smartButton->convertAmountToPayPalCurrency((float) $cart->tax_total, $cart->cart_currency_code);
        $shippingAmount = $this->smartButton->convertAmountToPayPalCurrency((float) ($cart->selected_shipping_rate ? $cart->selected_shipping_rate->price : 0), $cart->cart_currency_code);
        $discountAmount = $this->smartButton->convertAmountToPayPalCurrency((float) $cart->discount_amount, $cart->cart_currency_code);
        $grandTotal = $this->smartButton->convertAmountToPayPalCurrency((float) $cart->sub_total + $cart->tax_total + ($cart->selected_shipping_rate ? $cart->selected_shipping_rate->price : 0) - $cart->discount_amount, $cart->cart_currency_code);
        ...
        $data = [
            ...
            'purchase_units' => [
                [
                    ...
                    'items' => $this->getLineItems($cart, $paypalCurrencyCode),
                ],
            ],
        ];
     ...
     * @return array
     */
    protected function getLineItems($cart, $paypalCurrencyCode)
    {
        $lineItems = [];
        ...
        foreach ($cart->items as $item) {
            $lineItems[] = [
                'unit_amount' => [
                    'currency_code' => $paypalCurrencyCode,
                    'value' => $this->smartButton->convertAmountToPayPalCurrency((float) $item->price, $cart->cart_currency_code),
                ],
