@if (
    ...
    @php
        ...
        $paypalPayment = app(\Webkul\Paypal\Payment\SmartButton::class);

        $acceptedCurrenciesArray = array_map('trim', explode(',', $acceptedCurrency));

        $currencyToUse = $paypalPayment->getPaypalCurrencyCode($currentCurrency);

        if (! in_array($currencyToUse, $acceptedCurrenciesArray, true)) {
            $currencyToUse = $acceptedCurrenciesArray[0];
        }
    @endphp
