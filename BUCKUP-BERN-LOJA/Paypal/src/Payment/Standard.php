...
class Standard extends Paypal
{
    ...
    public function getFormFields()
    {
        ...
        $paypalCurrencyCode = $this->getPaypalCurrencyCode($cart->cart_currency_code);
        ...
        $fields = [
            ...
            'amount' => $this->convertAmountToPayPalCurrency((float) $cart->sub_total, $cart->cart_currency_code),
            'tax' => $this->convertAmountToPayPalCurrency((float) $cart->tax_total, $cart->cart_currency_code),
            'shipping' => $this->convertAmountToPayPalCurrency((float) ($cart->selected_shipping_rate ? $cart->selected_shipping_rate->price : 0), $cart->cart_currency_code),
            'discount_amount' => $this->convertAmountToPayPalCurrency((float) $cart->discount_amount, $cart->cart_currency_code),
        ];
    ...
    protected function addShippingAsLineItems(&$fields, $i)
    {
        ...
        $fields[sprintf('amount_%d', $i)] = $this->convertAmountToPayPalCurrency((float) $cart->selected_shipping_rate->price, $cart->cart_currency_code);
    }
}
